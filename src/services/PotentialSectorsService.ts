/**
 * PotentialSectorsService - 潜力板块服务
 * 提供六大潜力板块数据，支持公网部署
 * 
 * @module services/PotentialSectorsService
 * @version 1.0.0
 * @description 纯前端静态数据服务，无需后端API
 */
...
/**
 * 潜力板块数据 - 2026-03-19
 * 六大潜力板块：石油石化、金融控股、公用事业、多元金融、煤化工、门户网站
 */
const POTENTIAL_SECTORS_DATA: HotSector[] = [
  {
    code: 'BK0464',
    name: '石油石化',
    changePercent: 0.26,
...
/**
 * 获取潜力板块列表
 * @returns {HotSector[]} 六大潜力板块数据
 */
export function getPotentialSectors(): HotSector[] {
  return POTENTIAL_SECTORS_DATA;
}

/**
 * 获取板块详情
 * @param {string} code - 板块代码
 * @returns {HotSector | null} 板块详情
 */
export function getPotentialSectorByCode(code: string): HotSector | null {
  return POTENTIAL_SECTORS_DATA.find(sector => sector.code === code) || null;
}
 ...
 * @returns {any[]} 成分股列表
 */
export function getPotentialSectorStocks(sectorCode: string): any[] {
  const sector = POTENTIAL_SECTORS_DATA.find(s => s.code === sectorCode);
 ...
 * @returns {any[]} 股票列表
 */
export function getAllPotentialStocks(): any[] {
  const allStocks: any[] = [];
  const seenCodes = new Set<string>();
  
  POTENTIAL_SECTORS_DATA.forEach(sector => {
 ...
 * 服务导出
 */
export const potentialSectorsService = {
  getSectors: getPotentialSectors,
  getSectorByCode: getPotentialSectorByCode,
  getSectorStocks: getPotentialSectorStocks,
  getAllStocks: getAllPotentialStocks
};

export default potentialSectorsService;
